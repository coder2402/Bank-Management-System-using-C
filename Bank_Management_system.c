...
void view_list()
{
    ...
    // Optimization: Use fgets + sscanf instead of fscanf to avoid parsing unused fields.
    // This reduces CPU overhead significantly by only parsing necessary fields.
    char line_buffer[1024];
    while(fgets(line_buffer, sizeof(line_buffer), view) != NULL)
    {
        if (sscanf(line_buffer, "%d %s %*d/%*d/%*d %*d %s %*s %lf", &add.acc_no, add.name, add.address, &add.phone) == 4) {
